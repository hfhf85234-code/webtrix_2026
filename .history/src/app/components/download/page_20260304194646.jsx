    ...
    useEffect(() => {
      ...
      const ctx = gsap.context(() => {
        ...
        counters.forEach((counter) => {
          ...
          // التحقق إذا كان الرقم فيه فواصل الآلاف
          ...
          gsap.to(obj, {
            val: number,
            duration: 3.5, // المدة 3.5 ثواني
            ease: "expo.out", // easing احترافي
            snap: { val: 1 }, // تخلي الأرقام ثابتة بدون كسور
            
            // ربط الأنيميشن بـ ScrollTrigger
            scrollTrigger: {
              trigger: statsRef.current,
              start: "top 85%", // يبدأ لما يوصل 85% من الشاشة
              end: "bottom top",
              toggleActions: "play none none none", // play عند الدخول فقط
              once: true, // يتشغل مرة وحدة فحياته
              markers: false, // خليها true إذا بغيتي تشوف الـ triggers (للتصحيح)
              onEnter: () => {
                console.log("Animation started!"); // للتصحيح
              }
            },
            ...
            onUpdate: () => {
              if (hasCommas) {
                // إذا كان الرقم فيه فواصل (آلاف، ملايين)
                counter.innerText = Math.floor(obj.val).toLocaleString() + suffix;
              } else if (Number.isInteger(number)) {
                // إذا كان الرقم صحيح
                counter.innerText = Math.floor(obj.val) + suffix;
              } else {
                // إذا كان الرقم عشري (نعرض رقم عشري واحد)
                counter.innerText = obj.val.toFixed(1) + suffix;
              }
            },
      ...
      }, statsRef); // ربط الـ context بـ statsRef
      
      // تنظيف GSAP context و ScrollTrigger
      return () => {
        ctx.revert(); // إلغاء كل animations
        ScrollTrigger.getAll().forEach(trigger => trigger.kill()); // قتل كل triggers
      };
    }, []); // الـ use
