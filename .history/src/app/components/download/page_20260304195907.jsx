  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      counters.forEach((counter) => {
        ...
        // نحط 0 فالبداية
        counter.innerHTML = `<span class="digit">0</span>${suffix}`;
        ...
        gsap.to(obj, {
          ...
          onUpdate: () => {
            ...
            // نحول الرقم الحالي لحروف spans
            const digitsHTML = current
              .toString()
              .split("")
              .map(
                (digit) =>
                  `<span class="digit inline-block">${digit}</span>`
              )
              .join("");
  
            counter.innerHTML = digitsHTML + suffix;
  
            // animation لكل رقم جديد
            gsap.from(".digit", {
              yPercent: 100,
              duration: 0.6,
              stagger: 0.02,
